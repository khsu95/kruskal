...
int main()
{
	...
	{
		printf("Edge: ");
		ret = fscanf(inout, "%c %c %d\n", &temp.v, &temp.w, &temp.key);
		...
		//if(HEAP_INDEX>2) getchar();/*버퍼에 남은 데이터 초기화*/
	...
	fclose(inout);
	for (int i = 97; i <= 122; i++)/*정점의 개수만큼 Set을 만든다.*/
		set[i] = i;
	...
	{
		if (num_edge == -1||(selected_edge==num_edge-1))
		{
			printf("EMPTY TREE or Comlete MST \n");
			break;
		}
		temp = pop(&HEAP_INDEX);/*정렬된 MinHeap에서 최소 간선 꺼냄*/
		if (temp.v < 97 || temp.w < 97 || temp.key < 0) {
			printf("POP ERROR\n");	break;
		}
		...
		{
			Union(V_R, W_R);/*백년가약*/
			MST[selected_edge+1] = temp;/*선택 받은 i번째 간선*/
			selected_edge++;
		}
	}
	...
	inout = fopen("out.txt", "wt");/*바이너리 형식으로 저장할 것임*/
